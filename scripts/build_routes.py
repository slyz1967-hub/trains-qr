...
def main():
    ...
    # Compress and base64 encode each day separately so inject_routes.py
    # can update _TT_B64_MON / _TT_B64_FRI / _TT_B64_SAT / _TT_B64_SUN
    # individually in index.html.
    output = {
        'counts': {d: len(tt_db[d]) for d in tt_db},
    }
    total_b64_len = 0
    for day in tt_db:
        json_str = json.dumps(tt_db[day], separators=(',', ':'))
        compressed = zlib.compress(json_str.encode(), level=9)
        b64 = base64.b64encode(compressed).decode()
        output[f'b64_{day}'] = b64
    ...
    print(f'Total B64 length: {total_b64_len} chars')
    for d in tt_db:
        print(f'  {d}: {len(tt_db[d])} trains ({len(output[f"b64_{d}"])} b64 chars)')
