...
The route_db.json may contain either:
  - A single 'b64' key  (legacy single-variable format)
...
# Add more entries here if new day-splits are introduced.
DAY_KEY_MAP = {
    'b64_mon': '_TT_B64_MON',
    'b64_fri': '_TT_B64_FRI',
    'b64_sat': '_TT_B64_SAT',
...
# Single-variable aliases to try for legacy 'b64' key
...
def replace_var(html, var_name, new_value):
    """
    Replace the value of var_name in html. Returns (new_html, old_value) or
    raises ValueError with diagnostics if the variable isn't found.
    """
...
def inject(routes_path, html_path):
    ...
    # Collect (json_key, js_var_name, new_b64) tuples
    updates = []
    ...
    if day_keys:
        ...
        # Legacy single-variable format — try known alias names
        ...
        if not found_alias:
            ...
            names = [c[0] for c in candidates] if candidates else ['(none found)']
            raise ValueError(
                f'route_db.json has a single "b64" key but no known alias was found in index.html.\n'
                f'Long base64-like assignments present: {names}\n'
